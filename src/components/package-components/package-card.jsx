const PackageCard = ({ offer }) => {
  return (
    <div className="space-y-4 rounded-2xl border border-stone-400 p-4 text-stone-700 shadow md:max-w-100">
      <div className="py-4 font-semibold">
        <span className="text-sm">{offer.id}</span> <br />
        <h2 className="text-2xl text-amber-500">{offer.title}</h2>
      </div>
      <h3 className="rounded-2xl bg-linear-to-bl from-amber-100 to-amber-400 p-4 italic">
        <span className="font-semibold">Perfect for:</span> {offer.description}
      </h3>
      <div>
        <p>
